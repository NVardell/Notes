...
class FocusTrap extends React.Component {
  constructor(props) {
    ...
    this.internalOptions = {
      // We need to hijack the returnFocusOnDeactivate option, because React can move focus into the element before we arrived at this lifecycle hook (e.g. with autoFocus inputs). So the component captures the previouslyFocusedElement in componentWillMount, then (optionally) returns focus to it in componentWillUnmount.
      returnFocusOnDeactivate: false,

      // the rest of these are also related to deactivation of the trap, and we need to use them and control them as well
      checkCanReturnFocus: null,
      onDeactivate: this.handleDeactivate,
      onPostDeactivate: this.handlePostDeactivate,

      // we need to special-case this setting as well so that we can know if we should NOT return focus if the trap gets auto-deactivated as the result of an outside click (otherwise, we'll always think we should return focus because of how we manage that flag internally here)
      clickOutsideDeactivates: this.handleClickOutsideDeactivates,
    };

    // original options provided by the consumer
    this.originalOptions = {
      // because of the above `internalOptions`, we maintain our own flag for this option, and default it to `true` because that's focus-trap's default
      returnFocusOnDeactivate: true,

      // because of the above `internalOptions`, we keep these separate since they're part of the deactivation process which we configure (internally) to be shared between focus-trap and focus-trap-react
      onDeactivate: null,
    ...
    // if set, `{ target: Node, allowDeactivation: boolean }` where `target` is the outside node that was clicked, and `allowDeactivation` is the result of the consumer's option (stored in `this.originalOptions.clickOutsideDeactivates`, which may be a function) whether to allow or deny auto-deactivation on click on this outside node
    this.outsideClick = null;

    // elements from which to create the focus trap on mount; if a child is used instead of the `containerElements` prop, we'll get the child's related element when the trap renders and then is declared 'mounted'
   ...
   * @returns {Document|undefined} Configured document, falling back to the main document, if it exists. During SSR, `undefined` is returned since the document doesn't exist.
   */
  getDocument() {
  ...
  /**
   * Gets the node for the given option, which is expected to be an option that can be either a DOM node, a string that is a selector to get a node, `false` (if a node is explicitly NOT given), or a function that returns any of these values.
   * @param {string} optionName
   * @returns {undefined | false | HTMLElement | SVGElement} Returns `undefined` if the option is not specified; `false` if the option resolved to `false` (node explicitly not given); otherwise, the resolved DOM node.
